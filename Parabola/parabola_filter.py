...
def filter_locs(locs, min_ball_size, max_ball_size, root_dir, visual=False, video_dir=None, start_frame=None, end_frame=None,
                camera_id=None):
    ...
    if visual:
        show_locs(final_res, start_frame, end_frame, video_dir, camera_id,root_dir)
    return final_res


def show_locs(final_res, start_frame, end_frame, video_dir, camera_id, root_dir):
    cap = cv2.VideoCapture(video_dir)
    locs = []
    num = 1
    res_video_path = os.path.join(root_dir,"res_videos/res_{}.mp4".format(camera_id))
    ...
    res_video = cv2.VideoWriter(res_video_path, fourcc, float(fps), (int(width), int(height)), True)

    while True:
        ret, frame = cap.read()
        if not ret:
            break
        if end_frame > num >= start_frame:
            if final_res.get(num):
                found = cv2.circle(frame.copy(), final_res[num], 10, (0, 255, 0), 2)
                # show result
                # cv2.imshow("found", found)
                # cv2.waitKey(0)
                # save result into res_video
                res_video.write(found)
            num += 1
    return locs
