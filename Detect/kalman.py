...
def KalmanFilter():
    ...
    # diagonals
    kf.statePre = np.zeros((DP, 1), dtype=np.float32)
    kf.statePost = np.zeros((DP,1), dtype=np.float32)
    kf.transitionMatrix = np.eye(DP, dtype=np.float32)

    # x + vx + 0.5ax
    kf.transitionMatrix[0, 2] = 1
    kf.transitionMatrix[0, 4] = 0.5

    # y + vy + 0.5ay
    kf.transitionMatrix[1, 3] = 1
    kf.transitionMatrix[1, 5] = 0.5

    # vx + ax
    kf.transitionMatrix[2, 4] = 1

    # vy + ay
    kf.transitionMatrix[3, 5] = 1

    # predict ax = k * vx
    kf.transitionMatrix[4, 4] = 0
    kf.transitionMatrix[4, 2] = horizontal_acc_const

    # process noise cov matrix Q: models the EXTERNAL uncertainty
    kf.processNoiseCov = np.eye(DP, dtype=np.float32) * PN_Cov
    ...
    | 0 1 | y | = | y |
    '''
    kf.measurementMatrix = np.zeros((MP, DP), dtype=np.float32)
    kf.measurementMatrix[0, 0] = 1
    kf.measurementMatrix[1, 1] = 1

    # measurement noise cov matrix R: covariance of SENSOR noise
    kf.measurementNoiseCov = np.eye(MP, dtype=np.float32) * Sensor_Cov
    ...
    '''
    kf.errorCovPre = np.zeros((DP, DP), dtype=np.float32)

    kf.errorCovPost = np.eye(DP, dtype=np.float32)
    kf.errorCovPost[0, 2] = 1
    kf.errorCovPost[1, 3] = 1
    kf.errorCovPost[2, 0] = 1
    kf.errorCovPost[3, 1] = 1

    kf.gain = np.zeros((DP, MP), dtype=np.float32)

    return kf


# Manually set the KF post-state to initalise it
def setPostState(kf, x, y, vx, vy, ax, ay):
    setattr(kf, 'statePost', np.array([x,y,vx,vy,ax,ay], dtype=np.float32).reshape((6,1)))
...
def correct(kf, x, y):
    ...
    # temp2 = np.asmatrix(kf.measurementMatrix) * np.asmatrix(kf.errorCovPre)
    # print(temp2)
    #
    # temp3 = temp2 * np.asmatrix(kf.measurementMatrix).T + np.asmatrix(kf.measurementNoiseCov)
    # print(temp3)
    #
...
def verified(predicted_point, next_frame_index, v_distance):
    ...
    for point_index, point in enumerate(next_frame["x"]):
        cx = np.float32(next_frame["x"][point_index])
        cy = np.float32(next_frame["y"][point_index])
        c_pid = np.int(next_frame["pid"][point_index])
...
def point_is_near_point(point1, point2, dist):
    ...
    xdiff = np.float32(x1) - np.float32(x2)
    ydiff = np.float32(y1) - np.float32(y2)
...
def build_trajectory(this_trajectory, bridge, kf, frame_index, p0, p1, real):
    ...
    global n_miss
    postState = kf.statePost
    preState = kf.statePre
...
def get_data(filename):
    ...
    frame_array = [{} for x in range(max_frame + 1)]
...
if __name__ == '__main__':
    ...
    for frame_index, f0 in enumerate(frame_array):
        ...
        for b0_index, b0 in enumerate(f0["x"]):
            ...
            b0_x = np.float32(f0["x"][b0_index])
            b0_y = np.float32(f0["y"][b0_index])
            b0_pid = np.int(f0["pid"][b0_index])
            ...
            for b1_index, b1 in enumerate(f1["x"]):
                ...
                b1_x = np.float32(f1["x"][b1_index])
                b1_y = np.float32(f1["y"][b1_index])
                b1_pid = np.int(f1["pid"][b1_index])
                ...
                if sep < init_dist:
                    ...
                    setPostState(kf, b1[0], b1[1], vx, vy, 0, 0)
                    if d:
    ...
    time1 = time.time()
    print('cost {}s'.format(time1 - time0))
