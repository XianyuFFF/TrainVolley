...
def setPostState(kf, x, y, vx, vy, ax, ay):
    setattr(kf, 'statePost', np.array([x,y,vx,vy,ax,ay]).reshape((6,1)))
...
def correct(kf, x, y):
    ...
    temp3 = temp2* np.asmatrix(kf.measurementMatrix) + np.asmatrix(kf.measurementNoiseCov)
    print(temp3)
...
if __name__ == '__main__':
    ...
    measurement = np.zeros((2, 1), np.uint8)
    predicted = None
    corrected = None
